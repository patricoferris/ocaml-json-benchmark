 ...
 *)
 ...
 type t = [ `A of value list | `O of (string * value) list ]
 ...
 let json_of_src src : (value, [> read_value_error ]) result =
   ...
   let loc () = Jsone.decoded_range d in
   let dec () =
     match Jsone.decode d with
     | `Lexeme l -> l
     | `Error e -> raise (Abort (`Error (loc (), e)))
     | `End -> raise (Abort (`Unexpected `End_of_input))
     | `Await -> assert false
   in
   let rec value l stack =
     match l with
     | `Os -> obj [] stack
     | `As -> arr [] stack
     | (`Null | `Bool _ | `String _ | `Float _) as l -> continue l stack
     | _ -> raise (Abort (`Unexpected (`Lexeme (loc (), l, "value"))))
   ...
   and obj so_far stack =
     ...
     | `Name n ->
         ...
         value (dec ()) stack
     | l -> raise (Abort (`Unexpected (`Lexeme (loc (), l, "object fields"))))
 ...
 let value_to_dst ?(minify = true) dst json =
   ...
   and continue e stack =
     ...
     | Stack.Empty -> ()
   in
   let e = Jsone.encoder ~minify dst in
 ...
 let parse_error t fmt =
   Printf.ksprintf (fun msg -> raise (Parse_error (t, msg))) fmt
 ...
 let read_error_description : [< read_error ] -> string = function
   | `Error (_loc, err) -> Format.asprintf "%a" Jsone.pp_error err
   | `Unexpected `End_of_input -> Format.sprintf "Unexpected end of input"
 ...
 (* unit *)
 let unit () = `Null
 let get_unit = function `Null -> () | j -> parse_error j "Ezjsone.get_unit"

 (* bool *)
 let bool b = `Bool b
 let get_bool = function `Bool b -> b | j -> parse_error j "Ezjsone.get_bool"
 ...
 let get_string = function
   | `String s -> s
   | j -> parse_error j "Ezjsone.get_string"
 ...
 let get_int = function
   ...
   | j -> parse_error j "Ezjsone.get_int"

 let get_int32 = function
   | `Float f -> Int32.of_float f
   | j -> parse_error j "Ezjsone.get_int32"

 let get_int64 = function
   | `Float f -> Int64.of_float f
   | j -> parse_error j "Ezjsone.get_int64"
 ...
 let get_float = function
   | `Float f -> f
   | j -> parse_error j "Ezjsone.get_float"

 (* list *)
 let list fn l = `A (List.map fn l)

 let get_list fn = function
   | `A ks -> List.map fn ks
   | j -> parse_error j "Ezjsone.get_list"
 ...
 (* options *)
 let option fn = function None -> `Null | Some x -> `A [ fn x ]

 let get_option fn = function
   | `Null -> None
   | `A [ j ] -> Some (fn j)
   | j -> parse_error j "Ezjsone.get_option"

 (* dict *)
 let dict d = `O d
 let get_dict = function `O d -> d | j -> parse_error j "Ezjsone.get_dict"

 (* pairs *)
 let pair fk fv (k, v) = `A [ fk k; fv v ]

 let get_pair fk fv = function
   | `A [ k; v ] -> (fk k, fv v)
   | j -> parse_error j "Ezjsone.get_pair"

 (* triple *)

 let triple fa fb fc (a, b, c) = `A [ fa a; fb b; fc c ]

 let get_triple fa fb fc = function
   | `A [ a; b; c ] -> (fa a, fb b, fc c)
   | j -> parse_error j "Ezjsone.get_triple"
 ...
 let find_opt t path = try Some (find t path) with Not_found -> None
 ...
 let map f t path =
   let rec aux t = function
     ...
     | h :: tl -> (
         ...
         | _ -> None)
   in
   match aux t path with None -> raise Not_found | Some j -> j

 let update t path v = map (fun _ -> v) t path

 exception Not_utf8

 let is_valid_utf8 str =
   try
     Uutfe.String.fold_utf_8
 ...
 let decode_string_exn j =
   ...
   | Some s -> s
   | None -> parse_error j "Ezjsone.decode_string_exn"
 ...
 let t_of_sexp s =
   match value_of_sexp s with
   | `A x -> `A x
   | `O x -> `O x
   | _ ->
       failwith
         "Ezjsone: t_of_sexp encountered a value (fragment) rather than a t"
 ...
 let rec to_sexp json =
   ...
   | None -> (
       ...
       | _ -> parse_error json "Ezjsone.to_sexp")
